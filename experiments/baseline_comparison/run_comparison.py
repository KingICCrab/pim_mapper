...
from golden.trace_generator import DRAMConfig
from golden.workloads import get_paper_workloads
from golden.fast_trace_generator import FastTraceGenerator
...
def run_comparison():
    ...
    for i, workload in enumerate(workloads):
        if i < 10: continue # Skip existing workloads, run only new LLM ones
        ...
        # 4. Run Lemon (ILP without Row Activation)
        print("  Running Lemon Optimizer...", end="", flush=True)
        try:
            lemon_res = ilp_opt.optimize(
                [workload], 
                objective="latency",
                enable_row_activation=False
            )
            if lemon_res.mappings:
                lemon_mapping = lemon_res.mappings[0]
                lemon_stats = trace_gen.generate_trace(lemon_mapping, workload)
                lemon_max = max(lemon_stats.values())
                lemon_total = sum(lemon_stats.values())
                results["Lemon"] = {
                    "stats": lemon_stats, 
                    "max": lemon_max, 
                    "total": lemon_total
        ...
        with open(csv_path, 'a', newline='') as f:
            ...
            write_method_row("Lemon", results["Lemon"])
