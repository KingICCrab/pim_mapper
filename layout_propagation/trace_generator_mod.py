...
# Add parent directory to path to allow imports from layout_propagation
import sys
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
try:
    from layout_propagation.index_to_address import compute_permuted_strides
except ImportError:
    from index_to_address import compute_permuted_strides
...
class DRAMConfig:
    ...
    num_banks: int = 4
    num_rows: int = 16384
    # Row buffer size in bytes (matches arch.yaml)
...
class LayoutAwareTraceGenerator:
    """
    ...
    Modified to support Layout Propagation (decoupled Access Order and Data Layout).
    """

    def __init__(self, dram_config: DRAMConfig = None, target_layout_config: Dict = None):
        self.dram = dram_config or DRAMConfig()
        self.target_layout_config = target_layout_config
    ...
    def print_input_debug_info(self, num_entries: int = 50):
        ...
        for entry in self._input_debug_info:
            if prev_row is not None and entry['row'] != prev_row:
                row_switches.append(
                    (entry['idx'], prev_row, entry['row'], entry['h'], entry['w']))
            prev_row = entry['row']
    ...
    def generate_trace(self, mapping, workload, strict_ordering: bool = False) -> List[str]:
        ...
        dram_loops = self._build_dram_loop_structure(
            mapping, workload, buffer_tile)

        # Build tile_strides dict: {dim: tile_stride} for converting tile index to element coord
        # default stride=1 for non-tiled dims
        ...
        dram_loop_dims = [(loop_info['level'], loop_info['dim'],
                           loop_info['bound']) for loop_info in dram_loops]
        ...
        layout_info = self._compute_data_layouts(
            mapping, workload, H_in, W_in, buffer_tile)
        ...
        # Progress bar
        pbar = tqdm(total=total_iters, desc="Generating trace",
                    unit="tiles", leave=False)
        ...
        def iterate_dram_loops(level_idx: int, indices: Dict[Tuple[int, int], int]):
            ...
            for tile_idx in range(bound):
                ...
                # pure tile index for this (level, dim)
    ...
    def _compute_data_layouts(self, mapping, workload, H_in, W_in, buffer_tile: Dict[int, int]) -> Dict:
        ...
        dilation_h = workload.dilation[0] if hasattr(
            workload, 'dilation') else 1
        dilation_w = workload.dilation[1] if hasattr(
            workload, 'dilation') else 1

        # Input access tile dimensions (sliding window)
        H_per_tile = (P_per_tile - 1) * stride_h + \
            (R_per_tile - 1) * dilation_h + 1
        W_per_tile = (Q_per_tile - 1) * stride_w + \
            (S_per_tile - 1) * dilation_w + 1
        ...
        if input_layout == "row_aligned":
            input_aligned_tile_size = (
                (input_dram_tile_size + row_size - 1) // row_size) * row_size
        else:
        ...
        input_l3_dims_in_perm = [(lv, d)
                                 for (lv, d) in input_loop_order if lv == 3]

        # All Input-relevant dims at Level 3: P, Q, C, N (not K)
        input_l3_tile_counts = {DIM_P: P_l3,
                                DIM_Q: Q_l3, DIM_C: C_l3, DIM_N: N_l3}
        ...
        # C: stride = block_h * block_w
        input_strides[(2, DIM_C)] = block_h * block_w
        input_strides[(2, DIM_N)] = block_h * block_w * \
            C_per_tile  # N: full block size * C
        ...
        weight_sizes = {DIM_K: workload.K, DIM_C: workload.C,
                        DIM_R: workload.R, DIM_S: workload.S}
        weight_level2_tiles = {
            d: buffer_tile[d] * level2_factors[d] for d in weight_dims}
        weight_tile_sizes = {
        ...
        output_sizes = {DIM_N: workload.N, DIM_K: workload.K,
                        DIM_P: workload.P, DIM_Q: workload.Q}
        output_level2_tiles = {
            d: buffer_tile[d] * level2_factors[d] for d in output_dims}
        output_tile_sizes = {
        ...
        #
        bank_size = self.dram.row_buffer_bytes * \
            self.dram.num_rows  # 1024 * 16384 = 16MB
            ...
            if layout == "row_aligned" and level == 2 and dim in spatial_dims_2d:
                ...
                if not remaining_spatial_at_level2:
                    ...
                    if stride % row_size != 0:
                        padded_rb_tile_size = (
                            (stride + row_size - 1) // row_size) * row_size
                    else:
    ...
    def _build_dram_loop_structure(self, mapping, workload, buffer_tile) -> List[Dict]:
        ...
        """
        loops = []
        num_mems = max(mapping.loop_bounds.keys()) + \
            1 if mapping.loop_bounds else 0
        ...
        for m in [3, 2]:
            ...
            if m in mapping.permutation:
                ...
                sorted_perms = sorted(
                    perm.items(), key=lambda x: x[0], reverse=True)
    ...
    def _generate_tile_accesses(
        ...
        weight_tile_size = tile_size[DIM_K] * \
            tile_size[DIM_C] * tile_size[DIM_R] * tile_size[DIM_S]

        # Output tile size: N × K × P × Q (direct product, no special handling)
        output_tile_size = tile_size[DIM_N] * \
            tile_size[DIM_K] * tile_size[DIM_P] * tile_size[DIM_Q]
        ...
        dram_loop_dims = layout_info.get(
            'dram_loop_dims', [])  # [(level, dim, bound), ...]
        ...
        def get_accumulated_tile_index(dim: int) -> int:
            ...
            sorted_levels = sorted(level_bounds.keys(),
                                   reverse=True)  # Outer to inner
            multiplier = 1
        ...
        if generate_input:
            ...
            h_end = min((p_start + p_size - 1) * stride_h +
                        h_offset + (r_size - 1) * dilation_h + 1, H_in)
            w_start = q_start * stride_w + w_offset
            w_end = min((q_start + q_size - 1) * stride_w +
                        w_offset + (s_size - 1) * dilation_w + 1, W_in)
            ...
            if self.target_layout_config and 'input' in self.target_layout_config:
                # Input Target Layout Logic
                input_target = self.target_layout_config['input']
                if 'strides' not in input_target:
                    input_target['strides'] = compute_permuted_strides(
                        input_target['shape'], input_target['loop_order'])

                tgt_strides = input_target['strides']
                tgt_base = input_target.get('base_addr', 0)

                # Map dimensions to indices in shape (N, C, H, W)
                # Assuming shape is (N, C, H, W)
                IDX_N, IDX_C, IDX_H, IDX_W = 0, 1, 2, 3

                for n_local in range(min(n_size, workload.N - n_start)):
                    for c_local in range(min(c_size, workload.C - c_start)):
                        n_global = n_start + n_local
                        c_global = c_start + c_local

                        for h in range(h_start, h_end):
                            for w in range(w_start, w_end):
                                offset = (
                                    n_global * tgt_strides.get(IDX_N, 0) +
                                    c_global * tgt_strides.get(IDX_C, 0) +
                                    h * tgt_strides.get(IDX_H, 0) +
                                    w * tgt_strides.get(IDX_W, 0)
                                )
                                addr = tgt_base + offset * self.element_size
                                trace.append(f"RD 0x{addr:08X}")
            else:
                for n_local in range(min(n_size, workload.N - n_start)):
                    for c_local in range(min(c_size, workload.C - c_start)):
                        coords_iter = (
                            (h, w, h // block_h, w // block_w, h %
                             block_h, w % block_w)
                            for h in range(h_start, h_end)
                            for w in range(w_start, w_end)
                        )
        ...
        if generate_weight:
            ...
            if weight_layout == 'row_aligned':
                ...
                weight_tile_idx = compute_flat_tile_index_l3_only(
                    tile_indices, weight_relevant)
                ...
                for level, dim, bound in dram_loop_dims:
                    if level == 2 and dim in weight_relevant:
                        l2_tile_size *= bound
                weight_aligned_tile_size = (
                    (l2_tile_size + row_size - 1) // row_size) * row_size
                ...
                for level, dim, bound in reversed(dram_loop_dims):
                    if level == 2 and dim in weight_relevant:
                        l2_offset += tile_indices.get((level, dim), 0) * \
                            l2_stride * weight_tile_size
                        l2_stride *= bound

                weight_tile_base = weight_base + weight_tile_idx * weight_aligned_tile_size * \
                    self.element_size + l2_offset * self.element_size
            else:
                # Sequential: use all levels for flat_idx
                weight_tile_idx = compute_flat_tile_index(
                    tile_indices, weight_relevant)
                weight_aligned_tile_size = weight_tile_size
                weight_tile_base = weight_base + weight_tile_idx * \
                    weight_aligned_tile_size * self.element_size
        ...
        if generate_output:
            ...
            if output_layout == 'row_aligned':
                # For row_aligned: alignment is at Level 3, use only L3 dims for flat_idx
                output_tile_idx = compute_flat_tile_index_l3_only(
                    tile_indices, output_relevant)
                # L3 tile size = buffer_tile × L2_factors
                l2_tile_size = output_tile_size
                for level, dim, bound in dram_loop_dims:
                    if level == 2 and dim in output_relevant:
                        l2_tile_size *= bound
                output_aligned_tile_size = (
                    (l2_tile_size + row_size - 1) // row_size) * row_size
                ...
                for level, dim, bound in reversed(dram_loop_dims):
                    if level == 2 and dim in output_relevant:
                        l2_offset += tile_indices.get((level, dim), 0) * \
                            l2_stride * output_tile_size
                        l2_stride *= bound

                output_tile_base = output_base + output_tile_idx * output_aligned_tile_size * \
                    self.element_size + l2_offset * self.element_size
            else:
                # Sequential: use all levels for flat_idx
                output_tile_idx = compute_flat_tile_index(
                    tile_indices, output_relevant)
                output_aligned_tile_size = output_tile_size
                output_tile_base = output_base + output_tile_idx * \
                    output_aligned_tile_size * self.element_size

            if self.target_layout_config and 'output' in self.target_layout_config:
                output_target = self.target_layout_config['output']
                # Compute strides if not already computed
                if 'strides' not in output_target:
                    shape = output_target['shape']
                    order = output_target['loop_order']
                    output_target['strides'] = compute_permuted_strides(
                        shape, order)

                target_strides = output_target['strides']
                target_base = output_target.get('base_addr', 0)

                # Get tile indices
                n_tile = get_accumulated_tile_index(DIM_N)
                k_tile = get_accumulated_tile_index(DIM_K)
                p_tile = get_accumulated_tile_index(DIM_P)
                q_tile = get_accumulated_tile_index(DIM_Q)

                # Inner loop bounds (buffer tile sizes)
                N_inner = tile_size[DIM_N]
                K_inner = tile_size[DIM_K]
                P_inner = tile_size[DIM_P]
                Q_inner = tile_size[DIM_Q]

                # Iterate in Producer's order (N, K, P, Q inner)
                for n in range(N_inner):
                    for k in range(K_inner):
                        for p in range(P_inner):
                            for q in range(Q_inner):
                                # Global coordinates
                                n_global = n_tile * N_inner + n
                                k_global = k_tile * K_inner + k
                                p_global = p_tile * P_inner + p
                                q_global = q_tile * Q_inner + q

                                # Calculate address
                                # Map dimensions to indices in shape (N, K, P, Q)
                                # Assuming shape is (N, K, P, Q)
                                IDX_N, IDX_K, IDX_P, IDX_Q = 0, 1, 2, 3

                                offset_val = (
                                    n_global * target_strides[IDX_N] +
                                    k_global * target_strides[IDX_K] +
                                    p_global * target_strides[IDX_P] +
                                    q_global * target_strides[IDX_Q]
                                )
                                addr = target_base + offset_val * self.element_size
        ...
        def iterate(dim_idx: int, indices: Dict[int, int]):
            if dim_idx >= len(dim_order):
                idx = self._compute_tile_wise_address(
                    indices, loop_order, strides, tile_sizes_info)
