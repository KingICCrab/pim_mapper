...
def compute_dram_row_crossing_count(tile_bytes: float, row_bytes: float, num_tiles: int) -> int:
    ...
    if tile_bytes <= 0 or num_tiles <= 0:
        return 0
    # ！tile_bytes <= row_bytes. This case is illeagal, to delete.
...
def _build_outer_irrelevant_product(
    ...
    # ！！！！ We should think about which levels to include carefully.
...
def _build_sequential_dram_crossing(
    ...
    Streaming: L2 innermost loop is irrelevant, and DRAM outtermost loop is irrelevant.
    Thrashing: L3 innermost loop is irrelevant, or L2 outter loop is irrelevant with reuse.
    Thrashing: L3 irrelevant的位置会影响row activation的cost。即使row在多行。
